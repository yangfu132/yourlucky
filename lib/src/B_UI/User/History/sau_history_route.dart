...
class SAUHistoryRouteState extends State<SAUHistoryRoute> {
  ...
  Widget _rowWidget(SABEasyDigitModel model,int index) {
    ...
    ,delete: (position){
      ...
      SAUToastWidget.show("你点击了修改 $position");
    },);
  }
  ...
  void showLoginDialog() {
    ...
    showModalBottomSheet(context: context, builder: (context){
      return Container(height: 170,color: Colors.white,child: Column(children: <Widget>[
        ...
        const Text("删除后将无法看到该条记录，请谨慎操作",style: TextStyle(color: Colors.grey,fontSize: 14),),
        const SizedBox(height: 1,),
        Container(height: 50,width:double.infinity,margin:const EdgeInsets.only(left: 15,right: 15),
          child:  TextButton(onPressed:(){
            Navigator.of(context).pop();
            _deleteBank();
            SAUToastWidget.show("你点击了删除 $positionNow");
          },
            child: const Text("删除",style: TextStyle(fontSize: 16,color:Colors.blue),),),),
        const SizedBox(height: 10,),
        Container(height: 50,width:double.infinity,margin:const EdgeInsets.only(left: 15,right: 15),
          child:  TextButton(onPressed:(){Navigator.of(context).pop();}, child: const Text("取消",style: TextStyle(fontSize: 14),),),),
      ],),);
    });
  }
